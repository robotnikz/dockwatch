...
export default function Sidebar({
  ...
  const isBranchBuild = (() => {
    const v = appVersion?.currentVersion || '';
    return /^(main|sha|pr-\d+)-[0-9a-f]{7,40}$/i.test(v);
  })();

  const primaryVersionText = (() => {
    if (!appVersion) return '...';
    if (isBranchBuild && appVersion.latestVersion) {
      return `v${appVersion.latestVersion}`;
    }
    ...
    <aside className="w-[280px] shrink-0 border-r border-dock-border/50 bg-dock-card/60 flex flex-col">
      ...
      <div className="p-4 border-t border-dock-border/50 space-y-2">
        ...
        <div className="rounded-xl border border-dock-border/60 bg-dock-bg/40 px-4 py-3">
          ...
          <div className="mt-2 text-sm font-medium text-white">{primaryVersionText}</div>
          {appVersion && isBranchBuild ? (
            <div className="mt-1 text-[11px] text-dock-muted">Build: {appVersion.currentVersion}</div>
          ) : null}
        ...
        {authEnabled && authUsername ? (
          <div className="rounded-xl border border-dock-border/60 bg-dock-bg/40 px-3 py-3">
            <button
              type="button"
            ...
            >
              <div>
                <div className="text-[11px] uppercase tracking-wide text-dock-muted">User</div>
                <div className="text-sm font-semibold text-white">{authUsername}</div>
              </div>
              <span className="text-xs text-dock-muted">{menuOpen ? 'Hide' : 'Menu'}</span>
            </button>

            {menuOpen ? (
              <div className="mt-3 space-y-2">
                {authMessage ? (
                  <div className={`rounded-lg px-2 py-1.5 text-xs ${authMessage.tone === 'success' ? 'bg-dock-green/10 text-dock-green' : 'bg-dock-red/10 text-dock-red'}`}>
                    {authMessage.text}
                  </div>
                ) : null}
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-dock-border bg-dock-bg/70 px-2 py-1.5 text-xs text-white outline-none focus:border-dock-accent"
                />
                <input
                  type="password"
                  value={newPassword}
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (newPassword.length < 8) {
                      setAuthMessage({ tone: 'error', text: 'New password must be at least 8 characters.' });
                      return;
                    }
                    if (newPassword !== confirmNewPassword) {
                      setAuthMessage({ tone: 'error', text: 'New passwords do not match.' });
                      return;
                    }
                    ...
                    try {
                      ...
                      setAuthMessage({ tone: 'success', text: 'Password updated.' });
                    } catch (err: any) {
                      setAuthMessage({ tone: 'error', text: err?.message || 'Failed to change password' });
                    } finally {
                      setAuthBusy(false);
                    }
                  }}
                  className="w-full rounded-lg bg-dock-accent/20 px-2 py-1.5 text-xs font-semibold text-dock-accent transition hover:bg-dock-accent/30 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="w-full rounded-lg border border-dock-border px-2 py-1.5 text-xs font-semibold text-white transition hover:bg-dock-panel disabled:opacity-50"
                >
                  Logout
                </button>
              </div>
