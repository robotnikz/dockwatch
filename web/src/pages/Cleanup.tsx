...
export default function CleanupPage() {
  ...
  const resetStats = async () => {
    ...
    setMessage(null);
    try {
      await resetCleanupStatistics();
      setLiveLog('');
      setLastRun(null);
      setMessage({ tone: 'success', text: 'Cleanup statistics were reset.' });
      await refresh();
    } catch (err: any) {
      setMessage({ tone: 'error', text: err.message || 'Failed to reset statistics' });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <button
          ...
          className="rounded-xl border border-dock-red/40 bg-dock-red/10 px-5 py-3 text-sm font-bold text-dock-red transition hover:bg-dock-red/20 disabled:opacity-50"
        >
          Reset Statistics
        </button>
      </div>
      ...
      <section className="rounded-[24px] border border-dock-border/70 bg-dock-card/85 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatTile label="Image Refs" value={String(totals.images)} color="text-dock-accent" />
        ...
        <p className="mt-3 text-xs text-dock-muted">
          Image counters track deleted image references only and intentionally ignore raw SHA-only layer entries.
        </p>
      ...
      <section className="rounded-[24px] border border-dock-border/70 bg-dock-card/85 p-5 space-y-4">
        <button
          ...
          className="flex w-full items-center justify-between rounded-xl border border-dock-border/50 bg-dock-bg/20 px-4 py-3 text-left"
        >
          <h2 className="text-lg font-bold text-white">Cleanup Options</h2>
          <span className="text-sm font-medium text-dock-muted">{optionsExpanded ? 'Hide' : 'Show'}</span>
        </button>
      ...
      <ConfirmModal
        ...
        title="Reset cleanup statistics"
        message="This will permanently clear all cleanup run history and counters. Cleanup settings stay unchanged."
        confirmLabel="Reset"
        confirmTone="danger"
        busy={saving}
