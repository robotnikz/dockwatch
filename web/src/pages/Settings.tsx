...
import { getSchedulerEvents, getSettings, resetSchedulerEvents, saveSettings, testWebhook, type SchedulerEvent } from '../api';
...
export default function Settings() {
  ...
  const [schedulerEvents, setSchedulerEvents] = useState<SchedulerEvent[]>([]);
  ...
  useEffect(() => {
    Promise.all([getSettings(), getSchedulerEvents(10)])
      .then(([res, eventsRes]) => {
        setData(res);
        setSchedulerEvents(eventsRes.events);
  ...
  const handleResetSchedulerEvents = async () => {
    setResettingEvents(true);
    setMessage(null);
    try {
      await resetSchedulerEvents();
      setSchedulerEvents([]);
      setMessage({ tone: 'success', text: 'Scheduler diagnostics were reset.' });
    } catch (err: any) {
      setMessage({ tone: 'error', text: err.message || 'Failed to reset scheduler diagnostics' });
    } finally {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="rounded-2xl border border-dock-yellow/30 bg-dock-yellow/10 p-5">
          <h2 className="text-base font-bold text-dock-yellow">Security Notice (LAN Mode)</h2>
          <p className="mt-2 text-sm text-dock-text">
            DockWatch has high host privileges through Docker access. Keep it on trusted LAN/VPN only, do not expose it directly to WAN,
            and protect remote access via authenticated reverse proxy.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-dock-border/50 bg-dock-card p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              />
              <ToggleRow
                label="Status & Health alerts"
                description="Notify when containers stop, crash, or become unhealthy."
                checked={boolValue('discord_notify_status_changes', true)}
                onToggle={() => handleToggle('discord_notify_status_changes', true)}
              />
              <ToggleRow
                label="Scheduler error alerts"
                description="Notify on update/cleanup scheduler failures for faster troubleshooting."
                checked={boolValue('discord_notify_scheduler_errors', true)}
                onToggle={() => handleToggle('discord_notify_scheduler_errors', true)}
              />
            </div>
        ...
        <div className="rounded-2xl border border-dock-border/50 bg-dock-card p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            <h2 className="text-xl font-bold text-white">Scheduler Diagnostics</h2>
            <button
              type="button"
              onClick={handleResetSchedulerEvents}
              disabled={resettingEvents}
              className="rounded-lg border border-dock-border bg-dock-panel px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-dock-border disabled:opacity-50"
            >
              {resettingEvents ? 'Resetting...' : 'Reset diagnostics'}
            </button>
          </div>

          {schedulerEvents.length === 0 ? (
            <p className="text-sm text-dock-muted">No recent scheduler errors recorded.</p>
          ) : (
            <div className="space-y-2">
              {schedulerEvents.map((event) => (
                <div key={event.id} className="rounded-xl border border-dock-red/20 bg-dock-red/10 px-3 py-2">
                  <div className="text-xs text-dock-muted">{new Date(event.created_at).toLocaleString()}</div>
                  <div className="text-sm font-semibold text-dock-red">{event.category} {event.scope ? `(${event.scope})` : ''}</div>
                  <div className="text-sm text-dock-text break-words">{event.message}</div>
                </div>
              ))}
            </div>
          )}
