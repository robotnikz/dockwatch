...
function getDeletedSectionLines(output: string, header: string): string[] {
  const re = new RegExp(`${header}:\\n([\\s\\S]*?)\\n(?:Total reclaimed space:|$)`, 'i');
  const match = output.match(re);
  if (!match) return [];
  return match[1]
...
export function countDeletedImageReferences(output: string): number {
  const lines = getDeletedSectionLines(output, 'Deleted Images');
  if (lines.length === 0) return 0;

  const refs = new Set<string>();
  for (const line of lines) {
    const lower = line.toLowerCase();
    if (!lower.startsWith('untagged:')) continue;

    const ref = line.slice('untagged:'.length).trim();
    if (!ref) continue;

    // Ignore raw sha references to avoid counting layer-like digest entries.
    if (/^sha256:[a-f0-9]{12,}$/i.test(ref)) continue;
    refs.add(ref);
  }
...
async function getTopLevelImageInventoryCount(): Promise<number | null> {
  try {
    // `docker image ls -q` excludes internal layers and keeps counting focused on top-level images.
    const result = await execFileAsync('docker', ['image', 'ls', '-q'], { timeout: 20_000 });
    return new Set(
      result.stdout
        .split('\n')
        .map((line) => line.trim())
  ...
  try {
    ...
    if (dryRun) {
      ...
      if (config.options.images) {
        ...
        const imageCountBefore = await getTopLevelImageInventoryCount();
        emit(`-- image prune -a --\n$ docker ${imageArgs.join(' ')}\n`);
        const out = await runPruneCommand(imageArgs);
        outputs.push(`[image prune -a]\n${out}`);
        emit(`${out.trimEnd()}\n\n`);
        const imageCountAfter = await getTopLevelImageInventoryCount();
        const parsedDeletedRefs = countDeletedImageReferences(out);
        if (imageCountBefore != null && imageCountAfter != null) {
          // Inventory delta can miss digest-only untagged refs; keep the larger reliable count.
          const inventoryDelta = Math.max(0, imageCountBefore - imageCountAfter);
          deleted.images += Math.max(inventoryDelta, parsedDeletedRefs);
        } else {
          deleted.images += parsedDeletedRefs;
        }
        reclaimedBytes += parseReclaimedBytes(out);
      }
...
export function resetCleanupStatistics(): void {
  clearCleanupRuns();
}
