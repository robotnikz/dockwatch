...
async function getImageInventoryCount(): Promise<number | null> {
  try {
    const result = await execFileAsync('docker', ['image', 'ls', '-aq'], { timeout: 20_000 });
    return new Set(
      result.stdout
        .split('\n')
        .map((line) => line.trim())
  ...
  try {
    ...
    if (dryRun) {
      ...
      if (config.options.images) {
        ...
        const imageCountBefore = await getImageInventoryCount();
        emit(`-- image prune -a --\n$ docker ${imageArgs.join(' ')}\n`);
        const out = await runPruneCommand(imageArgs);
        outputs.push(`[image prune -a]\n${out}`);
        emit(`${out.trimEnd()}\n\n`);
        const imageCountAfter = await getImageInventoryCount();
        if (imageCountBefore != null && imageCountAfter != null) {
          deleted.images += Math.max(0, imageCountBefore - imageCountAfter);
        }
        reclaimedBytes += parseReclaimedBytes(out);
      }
