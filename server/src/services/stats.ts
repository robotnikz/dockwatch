...
export async function getAllContainerStats(): Promise<ContainerStats[]> {
  const [statsRes, psRes] = await Promise.all([
    ...
    execFileAsync('docker', [
      'ps', '-a', '--format', '{{.ID}}\t{{.Status}}\t{{.State}}'
    ], { timeout: 10_000 })
  ]);

  const psMap = new Map<string, { status: string; health: string; state: string }>();
  psRes.stdout.trim().split('\n').filter(Boolean).forEach(line => {
    const [id, statusStr, state] = line.split('\t');
    let health = '';
